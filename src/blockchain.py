...
class Block:
    def __init__(self, index, transaction, previus_hash):
        self.timestamp = time.time()
        self.index = index
...
def create_transaction(sender, receiver, amount, private_key_bytes):
    """
...
def verify_signature(sender_pub, tx: Transaction) -> bool:
    ...
    tx_data = json.dumps(tx.to_array(), sort_keys=True).encode('utf-8')
...
class BlockChain:
    def __init__(self):
        self.chain = []
    def create_block(self, previous_hash, transaction):
        """
        Create a new block and add it to the chain.
        """
        block = Block(len(self.chain) + 1, transaction, previous_hash)
